        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @elseif(auth()->user()->hasPermission('withdrawals.create'))
                        <a href="{{ route('withdrawals.create') }}" class="btn btn-primary">
                            <i class="fas fa-plus me-1"></i>
                            Tạo yêu cầu rút tiền
                        </a>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3">
                            ...
                            <select class="form-select" id="status" name="status" onchange="filterWithdrawals()">
                                ...
                                <option value="assistant_completed" {{ request('status') == 'assistant_completed' ? 'selected' : '' }}>Trợ lý đã hoàn thành thanh toán</option>
                                <option value="partner_confirmed" {{ request('status') == 'partner_confirmed' ? 'selected' : '' }}>Đối tác đã xác nhận nhận tiền</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-{{ auth()->user()->hasRole('partner') ? '6' : '3' }}">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-3">
                            <div class="card bg-warning text-dark">
                                <div class="card-body text-center">
                                    <h5>{{ $withdrawals->where('status', 'pending')->count() }}</h5>
                        ...
                        <div class="col-md-3">
                            <div class="card bg-info text-white">
                                <div class="card-body text-center">
                                    <h5>{{ $withdrawals->where('status', 'assistant_completed')->count() }}</h5>
                        ...
                        <div class="col-md-3">
                            <div class="card bg-success text-white">
                                <div class="card-body text-center">
                                    <h5>{{ $withdrawals->where('status', 'partner_confirmed')->count() }}</h5>
                        ...
                        <div class="col-md-3">
                            <div class="card bg-primary text-white">
                                <div class="card-body text-center">
                                    <h5>{{ number_format($withdrawals->sum('amount'), 0, ',', '.') }} VNĐ</h5>
                    ...
                    @if($withdrawals->count() > 0)
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @foreach($withdrawals as $withdrawal)
                                        <tr>
                                            <td>
                                                <a href="{{ route('withdrawals.show', $withdrawal) }}" class="text-decoration-none">
                                                    #{{ $withdrawal->id }}
                                                </a>
                                            </td>
                                            <td>
                                                <strong>{{ $withdrawal->partner->name }}</strong>
                                                <br><small class="text-muted">{{ $withdrawal->serviceProposals->count() }} đề xuất</small>
                                            </td>
                                            <td>
                                                <strong class="text-primary">{{ $withdrawal->formatted_amount }}</strong>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <br><small class="text-muted">{{ $withdrawal->created_at->diffForHumans() }}</small>
                                            </td>
                                            <td>
                                                @if($withdrawal->note)
                                                    <span class="text-truncate d-inline-block" style="max-width: 150px;" title="{{ $withdrawal->note }}">
                                                        {{ $withdrawal->note }}
                                                    </span>
                                                @else
                                                ...
                                                @foreach($withdrawal->getAvailableActionsFor(auth()->user()) as $action)
                                                    @if($action['action'] === 'assistant_process')
                                                        <button type="button" class="btn btn-sm {{ $action['class'] }}" 
                                                                ...
                                                                data-bs-target="#assistantProcessModal{{ $withdrawal->id }}">
                                                    ...
                                                    @elseif($action['action'] === 'partner_confirm')
                                                        <button type="button" class="btn btn-sm {{ $action['class'] }}" 
                                                                title="{{ $action['label'] }}"
                                                                data-bs-toggle="modal" 
                                                                data-bs-target="#partnerConfirmModal{{ $withdrawal->id }}">
                        ...
                        <div class="pagination-container">
                            <div class="pagination-info">
                                Hiển thị {{ $withdrawals->firstItem() }} - {{ $withdrawals->lastItem() }} 
                                trong tổng số {{ $withdrawals->total() }} yêu cầu rút tiền
                            </div>
                    ...
                    @else
                        <div class="text-center py-4">
                            ...
                            @if(auth()->user()->role && auth()->user()->role->name === 'partner')
                                <a href="{{ route('withdrawals.create') }}" class="btn btn-primary mt-2">
                                    <i class="fas fa-plus me-1"></i>
...
@foreach($withdrawals as $withdrawal)
    ...
    <div class="modal fade" id="assistantProcessModal{{ $withdrawal->id }}" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('withdrawals.assistant-process', $withdrawal) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <h6 class="fw-bold">Yêu cầu rút tiền #{{ $withdrawal->id }}</h6>
                            <p class="text-muted mb-3">
                                Đối tác: {{ $withdrawal->partner->name }}<br>
                                Số tiền: <strong class="text-primary">{{ $withdrawal->formatted_amount }}</strong>
                            </p>
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="payment_proof_image{{ $withdrawal->id }}" class="form-label">
                                <i class="fas fa-image me-1"></i>
                                Ảnh bill chuyển khoản <span class="text-danger">*</span>
                            </label>
                            <input type="file" 
                                   class="form-control" 
                                   id="payment_proof_image{{ $withdrawal->id }}" 
                        ...
                        <div class="mb-3">
                            <label for="assistant_note{{ $withdrawal->id }}" class="form-label">
                                <i class="fas fa-sticky-note me-1"></i>
                                Ghi chú
                            </label>
                            <textarea class="form-control" 
                                      id="assistant_note{{ $withdrawal->id }}" 
                                      name="assistant_note" 
                                      rows="3"
...
@foreach($withdrawals as $withdrawal)
    ...
    <div class="modal fade" id="partnerConfirmModal{{ $withdrawal->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                ...
                <form action="{{ route('withdrawals.partner-confirm', $withdrawal) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="partner_confirmation_note{{ $withdrawal->id }}" class="form-label">
                                <i class="fas fa-sticky-note me-1"></i>
                                Ghi chú xác nhận
                            </label>
                            <textarea class="form-control" 
                                      id="partner_confirmation_note{{ $withdrawal->id }}" 
...
function filterWithdrawals() {
    ...
    const url = '{{ route("withdrawals.index") }}' + (params.toString() ? '?' + params.toString() : '');
