                                    ...
                                    <div class="card-body">
                                                ...
                                                <table class="table table-hover">
                                                    ...
                                                    <tbody>
                                                        @foreach($serviceProposals as $proposal)
                                                            <tr class="{{ ($proposal->is_fully_withdrawn ?? false) ? 'table-secondary' : '' }}">
                                                                <td>
                                                                    @if($proposal->is_fully_withdrawn ?? false)
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <br><small class="text-muted">NCC: {{ $proposal->supplier_name }}</small>
                                                                </td>
                                                                <td>
                                                                    <strong class="text-primary">{{ $proposal->formatted_amount }}</strong>
                                                                    @if(($proposal->total_withdrawn ?? 0) > 0)
                                                                        <br><small class="text-info">
                                                                            <i class="fas fa-info-circle"></i> Đã rút: {{ number_format($proposal->total_withdrawn ?? 0) }} VNĐ
                                                                        </small>
                                                                    @endif
                                                                    @if(!($proposal->is_fully_withdrawn ?? false) && ($proposal->remaining_amount ?? 0) < $proposal->amount)
                                                                        <br><small class="text-success fw-bold">
                                                                            <i class="fas fa-coins"></i> Khả dụng: {{ number_format($proposal->remaining_amount ?? $proposal->amount) }} VNĐ
                                                                        </small>
                                                                    @endif
                                                                </td>
                                                                <td>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @else
                                                                        <div class="input-group">
                                                                            <input type="number" 
                                                                                   ...
                                                                                   name="amounts[{{ $proposal->id }}]" 
                                                                                   id="amount_{{ $proposal->id }}"
                                                                                   min="1" 
                                                                                   max="{{ $proposal->remaining_amount ?? $proposal->amount }}"
                                                                                   step="1"
                                                                                   placeholder="{{ number_format($proposal->remaining_amount ?? $proposal->amount) }}"
                                                                                   disabled
                                                                                   onchange="calculateTotal()">
                                                                            <span class="input-group-text">VNĐ</span>
                                                                        </div>
                                                                        <small class="text-success fw-bold">
                                                                            <i class="fas fa-hand-holding-usd"></i> Tối đa có thể rút: {{ number_format($proposal->remaining_amount ?? $proposal->amount) }} VNĐ
                                                                        </small>
                                                                    @endif
                                                                    ...
                                                                    @if(!($proposal->is_fully_withdrawn ?? false))
                                                                        <input type="hidden" 
                                                                               name="service_proposals[{{ $proposal->id }}][id]" 
                                                                               value="{{ $proposal->id }}">
                                                                    @endif
                                                                </td>
                                                            </tr>
                                        ...
                                        @else
                                            <div class="text-center py-4">
                                                ...
                                                <a href="{{ route('service-proposals.index') }}" class="btn btn-primary">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control @error('note') is-invalid @enderror" 
                                                      ...
                                                      name="note" 
                                                      rows="4"
                                                      placeholder="Ghi chú về yêu cầu rút tiền (không bắt buộc)">{{ old('note') }}</textarea>
                                            @error('note')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
